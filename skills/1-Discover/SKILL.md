...
- Append to `.claude/progress.md`: timestamp, `/discover`, step completed, key insights, next step.
...
step-05-bridge    → Produce a problem statement ready for /prd
```
...
5. At step-05 completion, tell the user: "Your discovery is complete. Run `/prd` — it will read `.claude/discovery-notes.md` and skip the front-door interview."
...
- PM already has a validated problem statement → skip to `/prd`
- PM is extending a known feature → skip to `/prd` or `/plan`
- PM has existing discovery notes in another format → paste them into context and run `/prd` directly

This skill is fully optional. Never block users from going directly to `/prd`.
